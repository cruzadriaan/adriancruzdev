    ...
    <div className="mb-10">
      ...
      {TechStackData.map((tech, index) => (
        <div key={index} className={`${containerClass} shadow-3xl mb-4`}>
          ...
          {/* Tech Category */}
          <div>
            <div className={`flex items-center gap-2 mb-2 ${tech.categoryColor}`}>
              <FontAwesomeIcon icon={tech.logo} className="text-2xl" />
              <h1 className="text-xl font-semibold">{tech.category}</h1>
            </div>
            ...
            <div className="flex items-center justify-start flex-wrap gap-7 mt-3">
              {tech.technologies.map((technology, index) => (
                <div key={index} className="flex flex-col items-center gap-1 hover:scale-110 transition-transform cursor-pointer">
                  ...
                  <h1 className="text-gray-200 text-[12px] font-semibold">{technology.name}</h1>
                </div>
              ))}
            </div>
